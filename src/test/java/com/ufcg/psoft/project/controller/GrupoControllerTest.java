...
@DisplayName("Testes do controlador de grupos de bolão")
public class GrupoControllerTest {
    ...
    @Test
    @DisplayName("Quando o organizador adiciona um participante a um grupo")
    ...
    @Test
    @DisplayName("Quando listar apenas os participantes de um grupo")
    ...
    @Test
    @DisplayName("Quando o organizador remove um participante do grupo")
    void quandoRemoverParticipanteComSucesso() throws Exception {
        grupo.getParticipantes().add(participante);
        grupoRepository.save(grupo);

        driver.perform(delete(URI_GRUPOS + "/" + grupo.getId() + "/participantes/" + participante.getId())
                .param("usuarioId", organizador.getId().toString())
                .param("codigoAcesso", organizador.getCodigo()))
                .andExpect(status().isNoContent());
    }
    ...
    @Test
    @DisplayName("Quando tenta adicionar um participante que não existe no sistema")
    ...
    @Test
    @DisplayName("Quando um participante tenta adicionar outra pessoa sem permissão")
    void quandoParticipanteTentaAdicionarOutroParticipante() throws Exception {
        driver.perform(post(URI_GRUPOS + "/" + grupo.getId() + "/participantes")
                .param("usuarioId", participante.getId().toString())
                .param("codigoAcesso", participante.getCodigo())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("Quando um participante tenta remover outro participante")
    void quandoParticipanteTentaRemoverOutroParticipante() throws Exception {
        driver.perform(delete(URI_GRUPOS + "/" + grupo.getId() + "/participantes/" + participante.getId())
                .param("usuarioId", participante.getId().toString())
                .param("codigoAcesso", participante.getCodigo()))
                .andExpect(status().isBadRequest());
    ...
    @Test
    @DisplayName("Quando listar os participantes de um grupo vazio")
    void quandoListarParticipantesGrupoVazio() throws Exception {
        driver.perform(get(URI_GRUPOS + "/" + grupo.getId() + "/participantes")
                .param("usuarioId", organizador.getId().toString())
                .param("codigoAcesso", organizador.getCodigo()))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("Quando tentar fazer chamadas sem passar o código de acesso")
    void quandoNaoInformarCodigoAcesso() throws Exception {
        driver.perform(get(URI_GRUPOS)
                .param("usuarioId", organizador.getId().toString()))
                .andExpect(status().isBadRequest());
    }
    ...
    @Test
    @DisplayName("Quando tentar fazer chamadas sem passar o id do usuário")
    void quandoNaoInformarIdUsuario() throws Exception {
        driver.perform(get(URI_GRUPOS)
                .param("codigoAcesso", organizador.getCodigo()))
                .andExpect(status().isBadRequest());
    }
